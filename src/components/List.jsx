...
export function List({ listname, user, setLists, listcards, list_id, setShowModal, listboard, toastNotify }) {
		...
		if (showInput && setNewCard.trim() !== "") {
			...
			api('createCard', 'POST', {
				...
				if (response.success === true) {
					toastNotify.success("Card Created!");
					...
					toastNotify.error("Failed to create card");
				}
			});
			// Optimistic update removal? No, the code below was duplicate logic or optimistic. 
			// Original code had `setCards([...cards, setNewCard.trim()]);` which is WRONG (string vs object).
			// Removing the duplicate optimistic update that pushes a string.
	...
	return (
		<div onClick={() => setSelectedCard(list_id)} onDrop={(e) => dropHandler(e)} onDragOver={(e) => dragOver(e)} className="rounded-xl p-4 border ml-2 max-w-sm h-fit min-w-sm max-h-[90%] overflow-y-auto bg-neutral-50 shadow-sm">
			<div className="flex flex-row justify-between text-lg mb-2 items-center">
				<h2>{listNameEdit ? <input type="text" value={newListName} onChange={(e) => {
					setNewListName(e.target.value);
				}} className="bg-transparent border-b focus:outline-none" /> : listname}</h2>
				<div onClick={() => {
					if (listNameEdit) {
						// save new list name
						api('renameList', 'POST', {
							...
							// console.log(response);
							if (response.success === true) {
								toastNotify.success("List Renamed!");
								setLists((prevLists) =>
									prevLists.map((lst) =>
								...
								toastNotify.error("Failed to rename list");
							}
						})
					}
